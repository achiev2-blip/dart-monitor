@echo off
chcp 65001 >nul
title News (Chain + Viewer 3300)

cd /d "%~dp0"
echo [INFO] News System Starting...
...
:: Kill existing port 3300
echo [CHECK] Port 3300...
for /f "tokens=5" %%p in ('netstat -ano ^| findstr ":3300 " ^| findstr "LISTENING" 2^>nul') do (
    echo [WARN] Port 3300 in use by PID %%p - killing...
...
start "News Chain" /min cmd /c "node news-chain.js"
...
echo.
echo ============================================================
echo   News Viewer (Port 3300)  [restart #%RESTART_COUNT%]
echo   Time: %DATE% %TIME%
