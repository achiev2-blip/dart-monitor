 ...
 * 뉴스 전용 뷰어 서버
 * 
 * ⚠️ 출력 전용 — 수집/분류는 매크로(news-collector.js, news-classifier.js)가 담당
 * 
 * 역할:
 ...
 * 
 * API:
 *   GET /api/news/feed   — 캐시된 뉴스 (호재/악재/강력호재/확인필요)
 *   GET /api/news/status — 캐시 상태 정보
 *   GET /                — 뉴스 뷰어 (public/)
 */
...
function loadCache() {
    if (!fs.existsSync(DATA_DIR)) {
        console.log('[뉴스-뷰어] data/output/ 폴더 없음');
        return;
    }

    // news_YYYYMMDD.json 파일들을 날짜 역순으로 정렬
    ...
    for (const file of files) {
        ...
        try {
            ...
            console.error(`[뉴스-뷰어] ${file} 읽기 실패: ${e.message}`);
        }
    }

    // 최신순 정렬 (pubDate 또는 _crawledAt 기준) → 300건 제한
    collected.sort((a, b) => {
        const da = new Date(a.pubDate || a._crawledAt || 0).getTime();
        const db = new Date(b.pubDate || b._crawledAt || 0).getTime();
        return db - da;
    });
...
app.get('/api/news/status', (req, res) => {
    ...
    const sourceCounts = {};
    cache.forEach(i => {
        const src = i.feedName || i.source || '기타';
...
// ── Claude 전용 엔드포인트 ──
// GET /api/claude/news           → 캐시에서 최신 300건 (일반 제외)
// GET /api/claude/news?limit=50  → 최신 50건만
// GET /api/claude/news?today=true → 오늘 파일 전체 (일반 포함)

app.get('/api/claude/news', (req, res) => {
    ...
    if (today) {
        ...
        const todayFile = path.join(DATA_DIR, `news_${dateStr}.json`);
        try {
    ...
    res.json({
        summary: `📰 뉴스 — ${source} (${items.length}건) | ${countStr || '없음'}`,
        total: items.length,
        counts,
        items: readable,
    });
...
app.listen(PORT, () => {
    console.log(`[뉴스 뷰어] http://localhost:${PORT}`);
