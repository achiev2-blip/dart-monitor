...
router.get('/claude/summary', (req, res) => {
    ...
    if (section) {
        ...
        return res.json({ ok: true, ai: 'claude', section, data: sectionData, _meta: dc._meta });
    }
...
function updateClaudeSummary(app) {
    try {
        ...
        // ── 상태형 데이터: 소스에서 읽어 DC에 저장 (참조만 넣으므로 가벼움) ──
        const watchlist = hantoo.getWatchlist();
        const stockPrices = hantoo.getStockPrices();
        ...
        // 뉴스   → news-dc.js     (dc.news)
        // 리포트 → reports-dc.js  (dc.reports)
        // 공시   → dart-dc.js     (dc.disclosures)
        // US     → us-dc.js       (dc.overseas)
