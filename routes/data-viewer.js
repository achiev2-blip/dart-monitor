...
// POST /api/data-file — JSON 파일 쓰기 (외부 API용)
...
router.post('/data-file', (req, res) => {
    ...
    if (!relPath.endsWith('.json')) return res.status(400).json({ error: '.json 파일만 쓰기 가능' });
    const full = path.join(DATA_DIR, relPath);
    if (!isSafePath(full)) return res.status(403).json({ error: '접근 불가' });
    try {
        const jsonStr = JSON.stringify(content, null, 2);
        if (jsonStr.length > 5 * 1024 * 1024) return res.status(413).json({ error: '5MB 초과' });
        const dir = path.dirname(full);
        if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
        fs.writeFileSync(full, jsonStr, 'utf-8');
        const stat = fs.statSync(full);
        res.json({ ok: true, path: relPath, size: stat.size, modified: stat.mtime.toISOString() });
